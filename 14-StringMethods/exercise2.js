...
//    let maxLength = Math.min(text.length, 40);
//    let minLength = Math.min(text.length, 30);
//    let start = Math.floor(Math.random() * (text.length - maxLength));
//    let end = start + minLength + Math.floor(Math.random() * (maxLength - minLength));
//     return text.substring(start, end);
//   }
//   let randomFragment=getRandomFragment( sampleNews);
